...
-- Try to clean old instance if exists (best-effort)
if _G.Hwan then
pcall(function()
if type(_G.Hwan) == "table" then
local oldGui = _G.Hwan.screenGui
if oldGui and oldGui.Parent then
pcall(function() oldGui:Destroy() end)
end
local oldConns = _G.Hwan.conns
if oldConns and type(oldConns) == "table" then
...
if _G.Hwan.Destroy and type(_G.Hwan.Destroy) == "function" then
pcall(function() _G.Hwan.Destroy() end)
end
end
end)
_G.Hwan = nil
...
-- subtitle support
if type(opts) == "table" and opts.Subtitle ~= nil then
    cfg.Subtitle = tostring(opts.Subtitle)
end
...
-- Subtitle
local Subtitle = nil
if cfg.Subtitle and #tostring(cfg.Subtitle) > 0 then
    ...
    -- subtle frame stroke around subtitle to simulate glow border
    local subStroke = new("UIStroke", {Parent = Subtitle})
    subStroke.Color = Color3.fromRGB(255,255,255)
    subStroke.Transparency = 0.75
    subStroke.Thickness = 1
    refs.Subtitle = Subtitle
end

local TabsFrame = new("Frame", {Parent = TitleFrame, Size = UDim2.new(1,0,0,36), Position = UDim2.new(0,0,0,56 + ((Subtitle and 0) or 0)), BackgroundTransparency = 1})
...
local function createTab(name)
    ...
    -- subtle stroke for selected state
    local btnStroke = new("UIStroke", {Parent = btn})
    ...
    new("UIPadding", {Parent = content, PaddingLeft = UDim.new(0,6), PaddingRight = UDim.new(0,6)})
    new("UIPadding", {Parent = content, PaddingTop = UDim.new(0,6)})
...
local function refreshSmallPanelContent()
    -- always update small label if exists and update btn.Text fallback
    local selText = ""
    if multiple then
        local vals = {}
        for i = 1, #options do
    ...
    -- fallback: update button text even if small panel not present
    pcall(function()
        if selText == "" then
            btn.Text = "Select"
        else
    ...
    if panelSmall and panelSmall.Parent then
        pcall(function()
            ...
            txtLabel.TextColor3 = cfg.Theme.Text
            ...
            local panelWidth = math.max(200, panelSmall.AbsoluteSize.X - 24)
            local bodyFont = Enum.Font.SourceSans
            local bodySize = 16
            local textSize = TextService:GetTextSize(selText, bodySize, bodyFont, Vector2.new(panelWidth, 2000))
...
local function computeFixedHeights()
    -- wait for Frame to have size
    local frameH = 400
    if Frame and Frame.AbsoluteSize and Frame.AbsoluteSize.Y and Frame.AbsoluteSize.Y > 0 then
        frameH = Frame.AbsoluteSize.Y
    end

    local dividerLocalY = dividerY
    if MainDivider and MainDivider.Parent and Frame and Frame.Parent then
        local ok, r = pcall(function() return (MainDivider.AbsolutePosition.Y - Frame.AbsolutePosition.Y) end)
        if ok and type(r) == "number" and r > 0 then dividerLocalY = r end
    end

    -- small panel: height from top to divider line (minus small padding)
    local smallH = math.floor(clampVal(dividerLocalY - 8, 56, math.max(56, math.floor(frameH * 0.45))))
    -- big panel: from divider line to bottom (minus small padding)
    local bigH = math.floor(clampVal(frameH - dividerLocalY - 8, 140, math.max(160, frameH - smallH - 8)))
...
local function updatePanelPos_fixed()
    ...
    pcall(function()
        ...
        -- reduce width a bit relative to main GUI
        local panelW = math.max(220, math.floor(cfg.Width * 0.78))
        ...
        -- add small padding from top of Frame
        local smallY = math.clamp(frameTopY + 8, 8, screenSize.Y - smallH - 8)
        local bigY = math.clamp(frameBottomY - bigH - 8, 8, screenSize.Y - bigH - 8)
        ...
        if panelSmall and panelSmall.Parent then panelSmall.Size = UDim2.new(0, panelW, 0, smallH) end
        if panelBig and panelBig.Parent then panelBig.Size = UDim2.new(0, panelW, 0, bigH) end
    end)
end

local function refreshSmallPanelContent_inner()
...
local function showPanel()
    ...
    selTextLabel.TextTransparency = 0
    ...
    -- add padding to contentFrame so items are centered properly
    new("UIPadding", {Parent = contentFrame, PaddingLeft = UDim.new(0,6), PaddingRight = UDim.new(0,6)})

    for i, opt in ipairs(options) do
        local rowBtn = new("TextButton", {Parent = contentFrame, Size = UDim2.new(1,-12,0,itemH-6), Position = UDim2.new(0,6,0, (i-1)*itemH), BackgroundColor3 = cfg.Theme.TabBg, Text = tostring(opt), Font = Enum.Font.SourceSansBold, TextSize = 17, TextColor3 = cfg.Theme.Text, AutoButtonColor = false, ZIndex = 222})
        new("UICorner", {Parent = rowBtn, CornerRadius = UDim.new(0,6)})
        rowBtn.TextStrokeTransparency = 1

        -- stroke to highlight selection
        local rowStroke = new("UIStroke", {Parent = rowBtn})
        ...
        rowBtn.MouseButton1Click:Connect(function()
            ...
            for _, child in ipairs(contentFrame:GetChildren()) do
                if child:IsA("TextButton") then
                    ...
                    if s then tween(s, {Transparency = 1}, 0.12) end
                end
            end
    ...
    -- restore previous selections (visual only)
    pcall(function()
                ...
                for childI, child in ipairs(contentFrame:GetChildren()) do
                    if child:IsA("TextButton") then
                        ...
                        if selectedSet[i] then
                            ...
                            local s = child:FindFirstChildOfClass("UIStroke")
                            if s then s.Transparency = 0.55 end
                        else
...
return instance
    ...
    function tab:CreateSlider(opts)
        ...
        local row = new("Frame", {Parent = content, Size = UDim2.new(1,0,0,54), BackgroundTransparency = 1})
        ...
        local leftLabel = new("TextLabel", {
            Parent = leftArea,
            Size = UDim2.new(1, -textPadding*2, 0, 18),
        ...
        local track = new("Frame", {Parent = leftArea, Size = UDim2.new(1,0,0,trackHeight), Position = UDim2.new(0,0,0,22), BackgroundColor3 = cfg.Theme.TabBg, BorderSizePixel = 0})
        ...
        local valueLabel = new("TextLabel", {Parent = rightBox, Size = UDim2.new(1, -12, 1, 0), Position = UDim2.new(0,6,0,0), BackgroundTransparency = 1, Font = leftLabel.Font, TextSize = leftLabel.TextSize, TextColor3 = Color3.fromRGB(245,245,245), TextXAlignment = Enum.TextXAlignment.Center, TextYAlignment = Enum.TextYAlignment.Center})
        ...
        local function setFromPercent(p, skipTween)
            ...
            if not skipTween then
                tween(fill, {Size = UDim2.new(p,0,1,0)}, 0.06, Enum.EasingStyle.Sine, Enum.EasingDirection.Out)
            else
                if fill then
                    fill.Size = UDim2.new(p, 0, 1, 0)
                end
            end
            if valueLabel then pcall(function() valueLabel.Text = tostring(displayValue) end) end
            ...
            if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                ...
                endedConn = input.Changed:Connect(function()
                    if input.UserInputState == Enum.UserInputState.End then
                        ...
                        local finalPct = 0
                        if track and track.AbsoluteSize and track.AbsoluteSize.X > 0 and fill and fill.AbsoluteSize then
    ...
    addConn(btn.MouseButton1Click:Connect(function()
        for _, t in ipairs(tabList) do
            ...
            if t and t.Button then
                ...
                if s then tween(s, {Transparency = 1}, 0.14) end
            end
        end
        ...
        if s then tween(s, {Transparency = 0.55}, 0.14) end
...
function window:SetSubtitle(text)
    ...
    if refs and refs.Subtitle then
        refs.Subtitle.Text = text
        refs.Subtitle.Visible = (text ~= "")
    else
        if not refs.Subtitle then
            ...
            local subStroke = new("UIStroke", {Parent = sub})
            subStroke.Color = Color3.fromRGB(255,255,255)
            subStroke.Transparency = 0.75
            subStroke.Thickness = 1
            refs.Subtitle = sub
...
_G.Hwan = { screenGui = screenGui, conns = conns, auth = false, Destroy = function() pcall(window.Destroy, window) end }
